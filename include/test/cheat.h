...
*/
__attribute__ ((__nonnull__))
static void cheat_exit(struct cheat_suite* const suite, __attribute__((unused)) int const status) {
	switch (suite->harness) {
	case CHEAT_UNSAFE:
...
static void cheat_isolate_test(struct cheat_suite* const suite,
	...
	if (pid == -1)
		...
		*/
		...
		fclose(suite->message_stream);
		suite->message_stream = NULL;
		cheat_clear_lists(suite);
...
#define CHEAT_TEST(name, ...) \
	static inline void CHEAT_GET(name)(void) { \
...
#define CHEAT_SET_UP(...) \
	static inline void cheat_set_up(void) { \
		__VA_ARGS__ \
	}

#define CHEAT_TEAR_DOWN(...) \
	static inline void cheat_tear_down(void) { \
		__VA_ARGS__ \
	}
...
__attribute__ ((__unused__))
static void CHEAT_WRAP(exit)(__attribute__((unused)) int const status) {
...
__attribute__ ((__unused__))
static void CHEAT_WRAP(_Exit)(__attribute__((unused)) int const status) {
	cheat_exit(&cheat_suite, CHEAT_EXITED);
}
